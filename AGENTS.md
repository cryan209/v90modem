...
Gotchas and repo-specific knowledge for this project. Kept in sync with `CLAUDE.md`; identical apart from this header.
...
- The Eicon Diva Server card work — the ADSP-2181 emulator, the MIPS firmware harness, the DIAL/INFO/V90D investigation tools and their 69-session analysis log — now lives in its own project, `../eicon-adsp-emu`. It shares no code with this one: it runs the card's shipped firmware under emulation, rather than implementing the spec. `tools/cx_at.py` and `docs/courier_firmware_analysis.md` stayed here, and that project references them.
...
- **V.90 analogue role — V.8 only** — `ME_V90_ROLE=analogue` makes V.8 offer the analogue side (only `pcm_modem_availability` flips to `_ANALOGUE`; `pstn_access` keeps `DCE_ON_DIGITAL` because it describes our access, not our role, and G.711-over-SIP *is* digital access; V.92 octet omitted). Nothing past V.8 exists: Phase 3 has no Sr/TRN1r/Jr and no Ja carrying a DIL descriptor, so the V.8 result handler logs what is missing and falls back to V.34 rather than putting a *digital* Sd/TRN1d/Jd on the wire from a modem that just announced itself as analogue. Default is unchanged. The measurement half is already built and tested (`v90_dil_measure.c`, `v90_dil_presets.c`) — `docs/v90_analogue_role.md`.
...
`CLAUDE.md` is the parallel copy of this file for Claude Code; mirror guidance changes there.
