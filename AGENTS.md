...
- **V.90 analogue role — V.8 only** — `ME_V90_ROLE=analogue` makes V.8 offer the analogue side (`pstn_access` 0, availability `_ANALOGUE`, V.92 octet omitted). Nothing past V.8 exists: Phase 3 has no Sr/TRN1r/Jr and no Ja carrying a DIL descriptor, so the V.8 result handler logs what is missing and falls back to V.34 rather than putting a *digital* Sd/TRN1d/Jd on the wire from a modem that just announced itself as analogue. Default is unchanged. The measurement half is already built and tested (`v90_dil_measure.c`, `v90_dil_presets.c`) — `docs/v90_analogue_role.md`.
