...
- **Receive path against a foreign downstream — Phase 3 reads, DIL does not** — every other receive test is fed by our own transmitter, so a wrong-but-self-consistent assumption is invisible to it. `make eicon-rx-test` points the analogue-side decoders at the Eicon card's downstream. Sd, S̄d, TRN1d (30005T) and Jd now decode at the offsets an independent segmentation puts them at, on both fixtures. **DIL does not decode**: `v90_dil_rx.c` acquires by finding an exactly-periodic run, but §8.4.1 gives every DIL-segment its own training Ucode, so a real DIL is periodic only at the full N-segment cycle — which a peer sends about once before terminating it. Our transmitter repeats cycles, which is why no loopback test sees this — `docs/eicon_downstream_comparison.md`, Finding 4.
