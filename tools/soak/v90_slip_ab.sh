#!/bin/bash
# Live A/B of the receive-path clock-recovery splice.
#
...
# ME_RX_CLOCK_SLIP=1 and restores the old behaviour.  Everything else -- one
...
#
#   v90_slip_ab.sh <outdir> [rate] [repeats] [attempts]
set -u
OUT=${1:?usage: v90_slip_ab.sh outdir [rate] [repeats] [attempts]}
RATE=${2:-19200}
REPEATS=${3:-3}
ATTEMPTS=${4:-3}
SP="$(cd "$(dirname "$0")" && pwd)"
...
for r in $(seq 1 "$REPEATS"); do
  for arm in fixed slip; do
    d="$OUT/$arm-r$r"
    [ -d "$d" ] && { echo "CONTROL: $arm r$r present, skipping"; continue; }
    if [ "$arm" = slip ]; then extra="ME_V90_UPSTREAM_MAX_BPS=$RATE ME_RX_CLOCK_SLIP=1"
    else                      extra="ME_V90_UPSTREAM_MAX_BPS=$RATE"; fi
    ...
    grep -a "RX clock-recovery slips" "$d/server.log" | head -1 | sed 's/^/  /'
  done
done

echo
echo "CONTROL: slip A/B, rate $RATE"
