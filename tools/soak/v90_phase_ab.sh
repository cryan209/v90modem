...
# ME_V90_PHASE_CONFIRM=0 and restores the old behaviour.  Everything else -- one
...
for r in $(seq 1 "$REPEATS"); do
  for arm in confirm single; do
    d="$OUT/$arm-r$r"
    [ -d "$d" ] && { echo "CONTROL: $arm r$r present, skipping"; continue; }
    if [ "$arm" = single ]; then extra="ME_V90_UPSTREAM_MAX_BPS=$RATE ME_V90_PHASE_CONFIRM=0"
...
echo
echo "CONTROL: phase-confirm A/B, rate $RATE"
