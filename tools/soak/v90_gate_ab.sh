...
# ME_V90_SWEEP_EYE_ABS=1 and restores the old behaviour.  Everything else -- one
...
for r in $(seq 1 "$REPEATS"); do
  for arm in relgate absgate; do
    d="$OUT/$arm-r$r"
    [ -d "$d" ] && { echo "CONTROL: $arm r$r present, skipping"; continue; }
    if [ "$arm" = absgate ]; then extra="ME_V90_UPSTREAM_MAX_BPS=$RATE ME_V90_SWEEP_EYE_ABS=1"
...
echo
echo "CONTROL: sweep-gate A/B, rate $RATE"
