...
reference = sign(received) * table[(idx-1) mod 6][ulaw(|received|)]
(decision-directed slicer against the TRN2-designed per-Ucode level table).
...
Reports, pure python (no numpy):
  ...
  - sign agreement sanity     -> must be 100% by construction
...
def main(path, win=655):
    ...
    # sign agreement sanity (reference takes sign from received)
    signdiff = sum(1 for a, b in zip(rx, ref) if (a < 0) != (b < 0) and b != 0)
    print("sign disagreements (must be 0): %d" % signdiff)
