// Standalone wrapper for MAME's ADSP-21xx disassembler (BSD-3-Clause).
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <cstdint>
...
public:
    const u8 *p; u32 len;
    u8 r8(u32 a) const { return p[a]; }
    u16 r16(u32 a) const { return p[a] | (p[a+1] << 8); }
    u32 r32(u32 a) const { return p[a] | (p[a+1] << 8) | (p[a+2] << 16) | ((u32)p[a+3] << 24); }
};
class disasm_interface {
public:
    virtual ~disasm_interface() = default;
    virtual u32 opcode_alignment() const = 0;
    virtual offs_t disassemble(std::ostream &s, offs_t pc, const data_buffer &o, const data_buffer &p) = 0;
};
}
template<typename T> inline u32 BIT(T x, int n, int w) { return (u32(x) >> n) & ((1u << w) - 1); }
template<typename T> inline u32 BIT(T x, int n) { return (u32(x) >> n) & 1; }

namespace util {
template<typename... A> void stream_format(std::ostream &s, const char *fmt, A... a) {
    char buf[256];
    // MAME uses %s/%d style identical to printf for these calls
    snprintf(buf, sizeof(buf), fmt, a...);
    s << buf;
}
}
namespace util { constexpr u32 STEP_OVER=1, STEP_COND=2, STEP_OUT=4, SUPPORTED=0x80000000; }
using util::data_buffer;
using util::STEP_OVER; using util::STEP_COND; using util::STEP_OUT; using util::SUPPORTED;
#include "2100dasm.h"

int main(int argc, char **argv)
{
    if (argc < 4) { fprintf(stderr, "usage: %s pm.bin <start> <end>\n", argv[0]); return 2; }
    FILE *fp = fopen(argv[1], "rb");
    if (!fp) { perror(argv[1]); return 1; }
    static uint32_t pm[0x4000];
    for (int a = 0; a < 0x4000; a++) {
        unsigned char w[3];
        if (fread(w, 1, 3, fp) != 3) break;
        pm[a] = w[0] | (w[1] << 8) | ((uint32_t)w[2] << 16);
    }
    fclose(fp);
    long start = strtol(argv[2], NULL, 0), end = strtol(argv[3], NULL, 0);
    adsp21xx_disassembler d;
    for (long a = start; a < end; a++) {
        util::data_buffer ops{reinterpret_cast<const u8*>(&pm[a]), 4};
        util::data_buffer params{reinterpret_cast<const u8*>(&pm[a]), 4};
        std::ostringstream s;
        d.disassemble(s, (offs_t)a, ops, params);
        printf("%04lx: %06x  %s\n", a, pm[a] & 0xffffff, s.str().c_str());
    }
    return 0;
}
