#!/usr/bin/env python3
"""Emit a V.32bis startup trace as exact complex symbols."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Emit V.32bis startup complex symbols")
...
def main(argv: list[str]) -> int:
    ...
    for index, symbol in enumerate(transmitted[: args.limit]):
        print(
            f"{index:05d} "
            f"{symbol.source_name:>12} "
            f"{symbol.symbol:>3} "
            f"{symbol.point.real:7.2f} "
            f"{symbol.point.imag:7.2f}"
        )
    return 0
