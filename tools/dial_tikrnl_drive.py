...
DIAL = 'artifacts/eicon-dsp/dial/0262-dial-fsk-fax.f34-overlay'
...
class Card:
    ...
    def _download(self, directory: str) -> None:
        base = REPO / directory
    ...
    def frame(self, rx_code: int, budget: int = 20000) -> collections.Counter:
        """One 8 kHz frame: present a line sample, run TIKRNL's frame loop."""
        self.dm[DM_LINE_RX] = rx_code & 0xFFFF
        hist: collections.Counter = collections.Counter()
        ADSP.adsp2181_call(self.cpu, FRAME_ENTRY, KERNEL_IDLE)
        for _ in range(budget):
            pc = ADSP.adsp2181_pc(self.cpu)
            hist[pc] += 1
            if pc not in self.pm_loaded:
                raise RuntimeError(f'ran into unpopulated PM at 0x{pc:04x}')
            ADSP.adsp2181_run(self.cpu, 1)
...
def main() -> int:
    ...
    for f in range(args.frames):
        hist = card.frame(tone[f % len(tone)])
        totals.update(hist)
    ...
    if download:
        ...
        # wanted overlay in DM 0x31AA with a type in DM 0x31A9.  A positive
        # table entry is requested directly; the negative entries (DIAL among
        # them) fall through to the fixed pair (type 0x000D, download 0x0270 =
        # the SIG overlay), which is what a host supervisor would serve next.
        print(f'[card] TIKRNL overlay request: DM 31AA=0x{download:04x} '
